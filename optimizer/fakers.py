...
class FakeAgents:
    ...
    @staticmethod
    def cv_structurer() -> Agent:
        """Fake agent that loads CV structure output from file"""
        return Agent(
            role="CV Structure File Reader",
            goal="Load CV structure output from file",
            backstory="A utility agent that reads pre-generated CV structure files",
            tools=[FileReadTool()],
            verbose=True,
        )
...
class FakeTasks:
    ...
    @staticmethod
    def cv_structuring_task(agent: Agent) -> Task:
        """Fake task that loads CV structure output from file"""
        return Task(
            description="""
            Use the FileReadTool to read the CV structure JSON file at {output_directory}/original_cv.json.
