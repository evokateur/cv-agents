...
class JobPostingWebScraper(BaseTool):
    ...
    def _run(self, url: str) -> JobPosting:
        try:
            headers = {"User-Agent": "CV-Optimizer/1.0 (Mozilla/5.0 compatible)"}
    ...
    def _parse_requirements_and_skills(
    ...
    def _extract_experience_level(self, soup: BeautifulSoup, description: str) -> str:
        ...
        if any(
            ...
            for word in ["entry level", "junior", "graduate", "0-2 years"]
        ):
            return "Entry Level"
        elif any(
            word in description_lower
            for word in ["senior", "lead", "principal", "5+ years", "7+ years"]
        ):
            return "Senior Level"
        elif any(
            word in description_lower
            for word in ["mid level", "intermediate", "3-5 years"]
        ):
            return "Mid Level"
        else:
